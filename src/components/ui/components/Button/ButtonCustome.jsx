...
const ButtonCustom = forwardRef((props,ref) => {
  ...
  const element = React.createElement(
    elementType,
    {
      ...
      className: classNames(
        buttonClasses({ color, size, loading, variant }),
        props.className
      ),
  ...
  return props.href ? (
    ...
    >
      <span className="block text-black dark:text-white">{element}</span>
    </Wrapper>
  );
});
...
const Wrapper = ({ children, tooltip, tooltipSide, tooltipOffset }) => {
  if (!tooltip) {
    return <>{children}</>;
  }
